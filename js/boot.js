...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.mozCancelFullScreen?document.mozCancelFullScreen().catch(e=>{}):null;
    window.elements = require("./base").elements
...
window.startSignup = ()=>{
    ...
    cloud.newAccount(elements.signupUname.value,elements.signupPw.value).then(e=>{
        console.log(e)
        if (e.error === "USERNAME_EXISTS"){
            // error
            elements.signupErrorDesc.innerHTML = "Already an account with the same name exists."
            elements.signupError.innerHTML = '<i data-feather="x-octagon"></i>  Conflict'
            transitionBootScreen(elements.bootScreen9, elements.bootScreen5)
            elements.signupError.parentElement.style.display = ""
            feather.replace()
        } else {
            cloud.getSession(elements.signupUname.value,elements.signupPw.value).then(e=>{
...
window.startLogin = ()=>{
    ...
    cloud.getSession(elements.loginUname.value,elements.loginPw.value).then(e=>{
        if (e.error === "UNAUTHORIZED"){
            // error
        } else {
            // throw onto desktop
...
function transitionBootScreen(currentElement, nextElement) {
...
const toggleFullScreen = () => {
    if (elements.bootScreen1.classList.contains('open')) {    
        ...
        transitionBootScreen(elements.bootScreen1, elements.bootScreen2)
